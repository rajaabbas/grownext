...
const MAX_ORGANIZATION_SLUG_LENGTH = 60;
const ORGANIZATION_SLUG_SUFFIX_LENGTH = 6;
const ORGANIZATION_SLUG_MAX_ATTEMPTS = 10;

const randomSlugSuffix = (): string => {
  let candidate = randomBytes(6).toString("base64url").replace(/[^a-z0-9]/gi, "").toLowerCase();

  while (candidate.length < ORGANIZATION_SLUG_SUFFIX_LENGTH) {
    candidate += Math.random().toString(36).replace(/[^a-z0-9]/gi, "").toLowerCase();
  }

  return candidate.slice(0, ORGANIZATION_SLUG_SUFFIX_LENGTH);
};

const prepareSlugBase = (value: string): string => {
  const normalized = slugify(value);
  if (normalized.length === 0) {
    return "org";
  }
  return normalized.slice(0, MAX_ORGANIZATION_SLUG_LENGTH);
};

const autoSlugCandidate = (base: string, attempt: number): string => {
  if (attempt === 0) {
    return base;
  }
  ...
  return slugify(`${trimmedBase}-${suffix}`);
};

const ensureAutoGeneratedSlug = async (
  tx: PrismaTransaction,
  source: string
): Promise<string> => {
  const base = prepareSlugBase(source);

  for (let attempt = 0; attempt < ORGANIZATION_SLUG_MAX_ATTEMPTS; attempt += 1) {
    const candidate = autoSlugCandidate(base, attempt);
  ...
  const fallback = slugify(`${base}-${randomUUID()}`);
  return fallback.length > 0 ? fallback.slice(0, MAX_ORGANIZATION_SLUG_LENGTH) : `org-${randomSlugSuffix()}`;
};

const ensureProvidedSlugAvailable = async (
  tx: PrismaTransaction,
  slugValue: string
): Promise<string | null> => {
  ...
  const existing = await tx.organization.findUnique({ where: { slug: normalized } });

  if (existing) {
    const error = new Error("organization_slug_in_use");
    error.name = "OrganizationSlugConflictError";
    throw error;
  }
...
export const createOrganizationWithOwner = async (
  ...
  const trimmedSlugInput = typeof input.slug === "string" ? input.slug.trim() : input.slug ?? undefined;
  ...
  return withAuthorizationTransaction(buildServiceRoleClaims(organizationId), async (tx) => {
    const organizationSlug =
      trimmedSlugInput === null
        ? null
        : await (typeof trimmedSlugInput === "string"
            ? ensureProvidedSlugAvailable(tx, trimmedSlugInput)
            : ensureAutoGeneratedSlug(tx, input.name));
...
export const findLatestOrganizationMembershipForUser = async (
  claims: SupabaseJwtClaims | null,
  userId: string
): Promise<OrganizationMember | null> => {
  return withAuthorizationTransaction(claims ?? buildServiceRoleClaims(undefined, { role: "service_role" }), (tx) =>
    tx.organizationMember.findFirst({
      where: { userId },
      orderBy: { createdAt: "desc" }
    })
